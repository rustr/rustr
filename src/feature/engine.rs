...
impl REngine {
    ...
    pub fn initialize(args: Vec<CString>, verbose: bool, interactive: bool) -> RResult<Self> {
		...
		unsafe {R_ReplDLLinit();} // populate the repl console buffers
		...
		rst.R_Interactive = if interactive {Rboolean::TRUE} else {Rboolean::FALSE};       // sets interactive() to eval to false
		rst.R_Verbose = if verbose {Rboolean::TRUE} else {Rboolean::FALSE}; 
		
        unsafe {R_SetParams(startr);}
